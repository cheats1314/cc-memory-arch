...
用户的"撤销/否定"语义必须**真正删除原条款**，不改写成"不再 X / 不喜欢 X / 不爱 X"否定式。否则等于把否定信息当新偏好持久化，造成认知噪声。

| 用户语义 | 触发词 | 动作 |
|---|---|---|
| **撤销**（无替代） | "我现在不 X 了 / 我不再 X / 取消 X / 别记 X 了 / 删除 X / 忘掉 X" | **从原条款剔除 X**；若整条 entry 因此变空，删除文件 + 同步索引；**不写"不再 X / 不 X"** |
| **替换** | "用 Y 不用 X / 改用 Y / X 改成 Y" | 把 X 替换为 Y，保留条目结构 |
| **新增** | "我喜欢 X / 加上 X / 还有 X" | append / 合并到现有条款 |

**反例**：用户说"我不爱吃辣了"时，原条款"喜欢吃辣，吃甜，吃零食"应改为"吃甜，吃零食"——**不要**改成"喜欢吃甜、零食（不再喜欢吃辣）"。后者把否定当新偏好记，等价于污染。

对应 frontmatter description / MEMORY.md 索引行：若条款删除导致原描述不再准确，同步缩短描述。
