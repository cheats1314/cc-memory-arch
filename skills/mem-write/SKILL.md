...
用户撤销旧偏好时，结果必须**看起来像 X 从未存在过**——文件、frontmatter、索引行任何位置都不应留下能让读者推断出"X 曾经被记过"的痕迹。

**单一检验标准（不通过就重写）**：

> 假装你从未读过原文件，仅根据用户**剩余的真实偏好**首次写入条款 —— 你产出的内容是否与撤销后的实际内容一致？
> - 一致 → 正确
> - 多出任何关于 X 的字（不论何种形式）→ 错

这个测试通过即合规，与具体措辞无关。

**为什么严格**：
- "不喜欢 X" 不是偏好，是偏好的**否定**。把否定持久化 = 把噪声当信号。
- 担心"用户之后又会加回 X" 是越权——那是下一轮对话的事。
- 撤销字段（如 frontmatter description、MEMORY.md 索引行）也要同步缩短，不能只改条款正文。

**工作步骤**（撤销时严格按此走，不要自由发挥）：
1. 读原条款，识别要删的项 X
2. **遗忘 X 曾经存在**
3. 用剩余项重写条款 / description / 索引行
4. 若 entry 因撤销变空 → 删文件 + 删索引行

**三种语义**：

| 用户语义 | 触发 | 动作 |
|---|---|---|
| **撤销**（仅否定，无替代） | "不 X 了 / 不再 X / 取消 X / 删 X / 别记 X / 忘掉 X" | 归零（按上述检验标准） |
| **替换**（否定 + 替代） | "用 Y 不用 X / 改用 Y / X 改成 Y" | X 换成 Y |
| **新增** | "还喜欢 X / 加上 X" | append / 合并 |

不要列穷举黑名单——任何"看起来在告诉读者 X 曾被记过"的写法都违反检验标准，无论它叫"不再 X"、"已不 X"、"曾经 X"、"~~X~~"、"（避免再加 X）"还是其他。
